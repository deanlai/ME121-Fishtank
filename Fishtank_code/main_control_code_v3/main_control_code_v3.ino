...
void loop() //------------------- LOOP ----------------------------------------------------------------------------
{
    ...
    const float cK1 = 0.0100;      //slope of change in heat v time when heater is on
    float tFrontDelay = 15000;    //delay from heater to being on to when the system temp starts to change
    float tEndDelay = 20000;      //time it takes residual heat in radiator to dissipate
...
//-----DO SOME MATH WITH SENSOR READINGS  
    ...
    heatTime = setHeaterTime(systemTemp, tLCL, tUCL, tSetpoint, cK1, tFrontDelay, tEndDelay);
...
float setHeaterTime(float temp, float LCL, float UCL, float setPoint, float K, float tFrontDelay, float tEndDelay) {
  float error;
  float heaterTime;
  if (temp<=LCL || (heaterState == 1 && temp<UCL)){ //if temp goes below LCL turn on heater, then keep it on until temp is @sp
    error = UCL - temp;
